 ...
 * The goal: find the partition where left half max ≤ right half min.
 *
 *
 * left half            | right half
 * ..................   | ..................
 * ... leftNums1        | pivotNums1 ...
 * ... leftNums2        | pivotNums2 ...
 * ... leftToPivotNums1 | pivotNums1 ...
 * ... leftToPivotNums2 | pivotNums2 ...
 *
 * if leftToPivotNums1 <= pivotNums2 && leftToPivotNums2 <= pivotNums1
 * even: median = (max of left half  + min of right half) / 2
 *              = Max(leftToPivotNums1, leftToPivotNums2) + Min(pivotNums1, pivotNums2) / 2
 *
 * odd:  median = max of left half (left has the extra element)
 *              = Max(leftToPivotNums1, leftToPivotNums2)
 *
 */
func findMedianSortedArrays(nums1 []int, nums2 []int) float64 {
	...
	for low <= high {
		...
		pivotIdxNums2 := (m+n+1)/2 - pivotIdxNums1 // handles the odd total length, median in left half

		leftToPivotNums1 := math.MinInt
		if pivotIdxNums1 > 0 {
			leftToPivotNums1 = nums1[pivotIdxNums1-1]
		}
		pivotNums1 := math.MaxInt
		if pivotIdxNums1 < m {
			pivotNums1 = nums1[pivotIdxNums1]
		}

		leftToPivotNums2 := math.MinInt
		if pivotIdxNums2 > 0 {
			leftToPivotNums2 = nums2[pivotIdxNums2-1]
		}
		pivotNums2 := math.MaxInt
		if pivotIdxNums2 < n {
